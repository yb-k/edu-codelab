...
`transition` 컴포넌트로 싸여진 엘리먼트가 삽입되거나 제거 될때 일어남.
...
css 애니메이션은 css 트랜지션과 같은 방식으로 적용됨.
차이점은 요소가 삽입된 직후에 `v-enter` 가 제거되지 않지만 `animationend` 이벤트가 있음.
...
`<transition>` 컴포넌트에 `duration` 속성 : 명시적 지속시간을 지정할 수 있음.
...
methods: {
  ...
  // done 콜백을 주자 않으면 동기적으로 호출되고, 트랜지션이 즉시 완료됨.
...
`appear` 속성을 추가해 초기 렌더에 트랜지션을 적용할 수 있음.
...
`appear` 속성이나, `v-on:appear` 둘다 최초 렌더링 시 트랜지션을 동작시 킬 수 있음.

### 엘리먼트 간 트랜지션

`v-if` /`v-else` 를 사용하여 원본 엘리먼트 사이를 트랜지션 할 수도 있음.
...
위 와 같이 **같은 테그 이름**을 가진 엘리먼트 사이를 트랜지션할 때, key 값을 설정해주는 것이 좋음.
...
실시간으로 업데이트 할 수 있으므로 프로토 타이핑에 특히 유용함.
예제 페이지 참고
